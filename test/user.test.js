    ...
    test('it should return error email is empty and status 400', (done) => {
      request(app)
        ...
        .end((err, response) => {
          // console.log('ini response',response.body)
          expect(err).toBe(null)
          expect(response.body).toHaveProperty('msg', 'Email can\'t be empty')
          expect(response.status).toBe(400)
          done()
        })
    })
    ...
    test('it should return error name is empty and status 400', (done) => {
      request(app)
        ...
        .end((err, response) => {
          // console.log('ini response',response.body)
          expect(err).toBe(null)
          expect(response.body.errors[0]).toHaveProperty('msg', 'User name can\'t be empty')
          expect(response.status).toBe(400)
          done()
        })
    })
    ...
    test('it should return error name is too short and status 400', (done) => {
      request(app)
        ...
        .end((err, response) => {
          // console.log('ini response',response.body)
          expect(err).toBe(null)
          expect(response.body.errors[0]).toHaveProperty('msg', 'User name min 3 characters')
          expect(response.status).toBe(400)
          done()
        })
    })
